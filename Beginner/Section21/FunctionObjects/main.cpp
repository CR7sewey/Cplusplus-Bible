...
struct Square_Functor {
    void operator() (int x) { // function object
            std::cout << x * x  << " ";
    }
};
...
int main()
{
    ...
    square(4);      // square.operator()(4) (whats really happening)    displays 16
    ...
    std::cout << "\nTest3 =================" << std::endl;
    std::for_each(vec1.begin(), vec1.end(), [](int x) {std::cout << x * x  << " "; });  // 1 4 9 16 25 --- closure env created in the compilor in the lambda
    std::cout << std::endl;
