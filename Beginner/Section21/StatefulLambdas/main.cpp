...
// here its like the variable passed in [] is stored as an atribute of the
// labda object during compile time (closure for the lambda with a parametrized ctor with atribute passed)
// this means that the operator method compiled is const (by defualt), so the var can not be changed! - see PP
...
void test1()
{
	...
	auto l = [local_x] () { //  capture by value (a copy (if an object a copy ctor is called)) (const value)
...
void test2()
{
	 ...
	 */
	 ...
	 // capture x - ctor
	auto l = [x] () mutable { // operator is not constant now, but it is passed by value (a copy) - ctor was called
		x += 100;
		std::cout << x << std::endl; 
	};
    
	l(); // 200 - operator called
	std::cout << x << std::endl; //100
    
    l(); // 300 - operator called - pq como x se tornou um atributo (39 - ver PP), e na primeira execucao ficou a 200, agora +é 200 + 100
    std::cout << x << std::endl; //100
}
...
void test3()
{
	...
	auto l = [&x] () mutable { // passed by reference
		x += 100;
		std::cout << x << std::endl;
	};
...
public:
	...
	// all capture by reference here - this is the better option
...
// Used for test9
class Lambda { // exactly like closure class created in compile time from a lambda expression
private:
	int y;
public:
	Lambda(int y) : y{y} {};
