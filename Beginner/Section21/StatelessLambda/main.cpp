...
void test2() {
    ...
    auto l1 =   [] () {std::cout << "Hi" << std::endl;}; // auto bcs the type is only generated when the lambda is compiled (check PP)
    l1();
...
void test3() {
    ...
    auto l4 = [] (Person p) { // by value - copy - copy constructor
        std::cout << p << std::endl; 
    };
    l4(stooge);
    
    auto l5 = [] (const Person &p) { // by reference - move contructor
        std::cout << p << std::endl; 
    };
    l5(stooge);
