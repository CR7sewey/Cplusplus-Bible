...
void test1() {
    ...
    arr1[0] = 1000; // no bounds checking - if out of bounds, 
    arr1.at(1) = 2000; // b.c. - if out, exceptionraised
    display(arr1);
...
void test3() {
    ...
    int *ptr = arr1.data(); // address of the first element of the array (similar to pointers iwoht raww array)
    std::cout << ptr << std::endl;
    *ptr = 10000;
    
    display(arr1); //[1000, 2, 3, 4, 5]
}
...
void test7() {
    ...
    int sum = std::accumulate(arr1.begin(), arr1.end(), 0); // reduce
