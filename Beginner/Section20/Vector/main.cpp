...
public:
    ...
    Person(std::string name, int age) 
        : name{name}, age{age}  {}
    bool operator<(const Person &rhs) const { // always override for all classes the < and == operator bcs they are used for a lot of operations like ordering and so on
...
void test2() {
    ...
    std::cout << "\nvec size: " << vec.size() << std::endl;
    std::cout << "vec max size: " << vec.max_size() << std::endl;
    std::cout << "vec capacity: " << vec.capacity() << std::endl; // previous * 2
    
    vec.shrink_to_fit();    // C++11
    display(vec);
    std::cout << "\nvec size: " << vec.size() << std::endl;
    std::cout << "vec max size: " << vec.max_size() << std::endl;
    std::cout << "vec capacity: " << vec.capacity() << std::endl; // vector size
...
void test4() {
    ...
    stooges.push_back(p1); // copy
    display(stooges);
    
    stooges.push_back(Person{"Moe", 25}); // move (rhv)
    display(stooges);
...
void test10() {
    ...
    std::transform(vec1.begin(), vec1.end(), vec2.begin(),
        ...
        [](int x, int y) { return x * y;}); // aka map
...
void test11() {
    ...
    display(vec1); // [1 2 3 4 100 200 300 4000 5 ...]
}
